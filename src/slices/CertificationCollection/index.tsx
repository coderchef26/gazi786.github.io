...
/**
 * Props for `CertificationCollection`.
 */
export type CertificationCollectionProps =
  SliceComponentProps<Content.CertificationCollectionSlice>;

interface CertificationCollectionItem {
  cert_name?: string;
  cert_acronym?: string;
...
/**
 * Component for "CertificationCollection" Slices - Collection of Certifications with Repeatable Zones.
 */
const CertificationCollection: FC<CertificationCollectionProps> = ({
  slice,
}) => {
  const { announce, speak, config } = useAtlas();
  const [certifications, setCertifications] = useState<CertificationCollectionItem[]>([]);
  ...
  useEffect(() => {
    const extractCertificationsFromCollection = () => {
      const certificationsData: CertificationCollectionItem[] = [];
  ...
  const handleCertificationClick = (cert: CertificationCollectionItem) => {
  ...
  const getCertificationStatus = (cert: CertificationCollectionItem) => {
